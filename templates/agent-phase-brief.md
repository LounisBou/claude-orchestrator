...
- You run at the **{{TIER}}** tier, chosen because {{TIER_REASON}}. Your session was spawned with the project's own servers only if the orchestrator passed `--mcp`, which it passes to an agent that drives a browser and to no other: do not expect a tool you were not given. If the work proves to need more judgment than this brief anticipated — a contract you would have to invent, an ambiguity two STOPs did not close — say so with the evidence and stop. The orchestrator escalates by replacing you with a fresh session one tier up; it cannot see from outside that the work outgrew the brief.
