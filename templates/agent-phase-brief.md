...
- Frontend surface: when the pull request creates or substantially modifies a frontend surface, or creates the interface of a new feature, drive that surface in a real browser with Playwright and put screenshots of it on the pull request, each naming the surface and the state it shows. A minor change (a colour, a spacing, a label, a fix invisible at a glance) is not held to it: say so and why in the report, and the orchestrator rules.
...
- You run at the **{{TIER}}** tier, chosen because {{TIER_REASON}}. Your session was spawned with these servers and no other: {{MCP_SERVERS}} — do not expect a tool you were not given. If the work proves to need more judgment than this brief anticipated — a contract you would have to invent, an ambiguity two STOPs did not close — say so with the evidence and stop. The orchestrator escalates by replacing you with a fresh session one tier up; it cannot see from outside that the work outgrew the brief.
