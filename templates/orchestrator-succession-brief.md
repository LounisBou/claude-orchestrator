...
3. Run `ListAgents`. Message every live implementer (names like `{{AGENT_NAME_PATTERN}}`): identify yourself as the new orchestrator BY YOUR EXACT `ListAgents` NAME AND REFERENCE — copy it from the listing, the agents will address it verbatim — ask for a one-line status and a measured context (`claude-orchestrator:context-gauge`), and subscribe to each one's idle notice (`notify_when_idle: true`). Their standing protocol carries over unchanged, with the new address in place of the old.
