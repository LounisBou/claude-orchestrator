...
4. Message the predecessor "takeover confirmed" and wait for its « handed over » (its last message; five minutes without it, read its screen with `screen --tty` and close on a prompt with nothing in flight). Then CLOSE ITS TAB with `orchestrator:iterm-agents` (`list` first; close by tty with `--expect-title`) once it reports idle — never while it may still be writing, and never leave it open: a predecessor that stays alive answers agents that address it by habit. This step is not optional and a brief that drops it is defective.
