...
| `/orchestrator:audit` | Launches, on the operator's word, an auditor of the orchestration: a read-only session in a tab titled `Audit : <subject>`, beside the orchestrator, on its model and under remote control, in no chain. It reads the deliveries, the orchestrator's conduct and what is due, reports to the operator in a report of fixed shape, and orders the orchestrator to tighten or loosen the method, each change with its measurement. It also makes the orchestration advance: it names the waits that need no word, answers the orchestrator's decisions with one recommendation and its cost, and pre-digests the operator's so that they take one word. And it maintains the project's method-and-decisions file — the operator's method in the operator's words, dated decisions, the orders and their fate — recorded per project in the state directory so that the next audit finds it, and landed by the orchestrator where the project keeps it. |
| `/orchestrator:audit-end` | Ends the audit on the operator's word only: the operator types it, and no session runs it by itself. Until then the auditor only invites the operator to end the audit, and waits. In the auditor's tab it sends the report path and the orders and ends the auditor's turn; in the orchestrator's, it acknowledges them, closes the auditor's tab and clears the record. The report stays on disk for the next audit. |
