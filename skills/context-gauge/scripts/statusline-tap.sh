...
record() {
  local fields sid ctx used total h5 h5r d7 d7r file tmp
  fields=$(printf '%s' "$input" | jq -r '
    [ (.session_id // ""),
      (.context_window.used_percentage // null),
      (.context_window.used // null),
      (.context_window.total // null),
  ...
  tmp="$file.tmp.$$"
  printf '{"session_id":"%s","context_percent":%s,"context_used":%s,"context_total":%s,"five_hour_percent":%s,"five_hour_resets_at":%s,"seven_day_percent":%s,"seven_day_resets_at":%s,"updated_epoch":%s}\n' \
    "$sid" "$ctx" "$used" "$total" "$h5" "$h5r" "$d7" "$d7r" "$(date +%s)" > "$tmp" 2>/dev/null \
