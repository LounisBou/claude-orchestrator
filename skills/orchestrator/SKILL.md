...
**Ready is the operator's turn, and the pull request stays in draft.** Ready, from your side, is: implemented, nothing pending, no decision waiting, the pull request open in DRAFT, its review round and norms check done, its correction round made and verified, `ready` green at that head — and the branch rebased, conflicts resolved by you: on the main branch, and each pull request of a stack on the one below it. Then, and not before, you tell the operator « ready »: what is left is his review, taking it out of draft and approving the squash-merge, and none of the three is yours. `ready` is read at the verified head, BEFORE the rebase, since the rebase moves the head without adding anything to review; a rebase whose conflicts you resolved is verified like the correction round, by you — `git range-diff` against the verified head shows the resolution and nothing else, and the gate runs on the new head. **The squash-merge trap**: once a lower pull request of a stack is squash-merged, its commits exist on the main branch under no name you can rebase over; the next branch is rebased by replaying ONLY its own commits onto the main branch — `git rebase --onto <main> <old head of the lower branch> <branch>` — never the lower branch's original commits, which come back as conflicts or as duplicates. The rebase is pushed with `--force-with-lease` against the head you read, the one force this rule allows; any other push stays plain.
