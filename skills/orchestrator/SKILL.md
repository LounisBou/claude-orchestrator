...
- **A frontend surface is proved by a browser run and by screenshots on the pull request.** A pull request that creates or substantially modifies a frontend surface, or creates the interface of a new feature, is proved by testing that surface in a real browser with Playwright and by screenshots of it on the pull request, and the report names the Playwright run. A minor change — a colour, a spacing, a label, a fix invisible at a glance — is not held to it: when the implementer judges a change minor, the report says so and why, and the orchestrator rules. The screenshots go in the pull request's description, written by the implementer; they are never committed to the branch; they are taken on fixtures or seeded data, with no secret, token, personal data, internal host or local path visible.
...
12. **A pull request that creates or substantially modifies a frontend surface, or creates the interface of a new feature, is not read on its diff alone.** Before its verdict, check that the screenshots are on the pull request, that they show the surface the diff changes, and that the implementer's report names the Playwright run: a missing or unrelated screenshot is a finding, and the pull request is not ready. Where the implementer called the change minor, its report says why and you rule.
