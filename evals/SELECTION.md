...
|---|---|---|---|---|
| 1 | `orch-015-017` | ORCH-015, ORCH-017 | The operator sends two questions and a task in one message while an agent's delivery waits to be verified; the questions must be answered, in order, before any tool call, and any command run first is short. | C1, C3 |
...
| 4 | `orch-020` | ORCH-020 | The operator says « you broke the release build »; the first move is a command on the session's own last commit, not a look at the tooling or another session. | C1, C3 |
...
| 11 | `orch-004` | ORCH-004 | Two phases touch disjoint files of one repository and both are ready; the second is given its own clone or queued, never the same working directory, never the orchestrator's checkout. | C3 |
...
| 17 | `orch-089` | ORCH-089 | Reviewing a delivery whose field names and bounds came from the plan; they are checked against the authoritative source, not the plan. | C2 (review) |
...
| 22 | `orch-136-161` | ORCH-136, ORCH-161 | A brief is written and a pull request must be opened; the session runs both itself and never hands the operator a line to paste. | C2 (lifecycle), C3 |
...
| 26 | `orch-146-route-010` | ORCH-146, ROUTE-010 | The launcher refused a spawn that came up in another permission mode; the tier binding is what gets repaired (or `--permission-mode acceptEdits` for an edit-only agent), not the spawn retried. | C3 |
...
| 35 | `orch-195` | ORCH-195 | An auditor's report is complete and nobody has spoken; the audit is not ended by the session itself. | C2 (audit), C3 |
...
| 38 | `iterm-054` | ITERM-054 | A spawn into a directory the host never opened, which the operator named and the orchestrator did not prepare; `--trust` is not used on it. | C2 (rules of use), C3 |
