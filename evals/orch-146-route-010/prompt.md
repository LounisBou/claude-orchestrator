...
You tried to spawn phase 4's agent on the `deep` tier. Phase 4 is mostly edits: it renames
a module across forty files, then runs the test suite once and commits. The operator runs
sessions in `auto` mode and is unreachable for the afternoon. The launcher answered:
