...
Grades ITERM-054 (a directory the host never opened stops the session on a workspace
question: `spawn` refuses it before making a tab, and `--trust` records the answer for one
directory, only for a checkout the orchestrator prepared).
