...
and the `limit` bound, and the parameter names) against the upstream partner service's own contract (the one vendored under `vendor/`, or
the upstream service itself), not only against
