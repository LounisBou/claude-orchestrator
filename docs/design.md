...
**iterm-agents** is unchanged in behaviour: `list`, `spawn`, `close`, `move`, `rotate`, with `--left-of` on `spawn` and `rotate`. The layout convention is stated: the orchestrator's tab sits immediately left of its implementer's tab. The spawn prompt is a one-line "Read and execute <path>".
