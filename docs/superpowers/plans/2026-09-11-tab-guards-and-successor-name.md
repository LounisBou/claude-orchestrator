...
- Remote control: `--successor` adds `--remote-control <the successor's name>` to the launch unless `--no-remote-control`; a spawn without `--successor` never adds it, and `--remote-control` is not a spawn option (a fresh orchestrator is the operator's hand: `--remote-control "Orchestrator : <feature>"`, said in the rulebook's first-instantiation sentence, Task 3).
...
**Files:**
- Modify: `skills/orchestrator/SKILL.md` — lifecycle step 4 (the stand-down refusal), the review-rounds paragraph (the gauge path sentence), the succession paragraph's first-instantiation sentence (`--remote-control "Orchestrator : <feature>"` beside `--name` for a session the operator starts by hand).
